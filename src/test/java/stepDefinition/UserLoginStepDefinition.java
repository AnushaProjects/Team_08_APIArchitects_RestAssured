...
public class UserLoginStepDefinition extends ReusableVariables{
	...
	public void admin_admin_receives_created_with_auto_generated_token(int successcode) {
	...
	public void admin_admin_receives_admin_receives_unauthorized(int errorcode) {
		System.out.println(loginResponse.statusCode());
		Assert.assertEquals(loginResponse.statusCode(), errorcode);
	}

	@Then("Admin Admin receives Admin receives {int} BadRequest")
	public void admin_admin_receives_admin_receives_bad_request(int badrequest) {
		System.out.println(loginResponse.statusCode());
